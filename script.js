window.onload = function () {
    ...
    // Função para gerar uma posição central aleatória
    function randomCentralPosition() {
        const centerX = Math.floor(area.width / 2 / tp); // Centro horizontal
        const centerY = Math.floor(area.height / 2 / tp); // Centro vertical
        const randomX = Math.floor(Math.random() * (centerX - 1)) + 1; // Gera um valor aleatório dentro da metade do campo
        const randomY = Math.floor(Math.random() * (centerY - 1)) + 1;
        return { x: randomX, y: randomY };
    }

    // Eixo inicial da Maçã
    let { x: applex, y: appley } = randomCentralPosition();
    ...
    function game() {
        ...
        for (let i = 0; i < trail.length; i++) {
            ...
            if (trail[i].x == px && trail[i].y == py)
            {
                ...
                tail = 2
                pontoi = 0
                pontos.innerHTML = pontoi; // Zere a pontuação
        ...
        if (applex == px && appley == py) {
            tail++
            ({ x: applex, y: appley } = randomCentralPosition()); // Gere uma nova posição central
